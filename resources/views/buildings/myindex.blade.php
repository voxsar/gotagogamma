        ...
        @forelse ($buildings as $building)
            <tr>
                ...
                @forelse ($building->costs as $cost)
                    <td>{{(($cost->pivot->cost * $building->pivot->level) * $building->multiplier) / 100}}</td>
                @empty

                @endforelse
                <td scope="col">
                    <i class="bi bi-alarm"></i> {{Carbon\Carbon::parse($building->base * $building->pivot->level)->format('H:i:s')}}
                </td>
                <td scope="col">
